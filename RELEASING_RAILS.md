...
However, due to that Action Cable also maintains an `npm` package here are the additional build steps that are run to release it:

```
# This is run as apart of the actioncable:update_versions rake task
npm version #{version} --no-git-tag-version

# This is run as apart of the actioncable:push rake task
npm publish
```
